...
3. 调用 `scripts/generate_pptx.py` 生成最终文件：
   ```bash
   python scripts/generate_pptx.py --input ./ppt_data.json --output ./output/presentation.pptx
   ```
4. 确认文件生成成功，告知用户
...
- 命令：`python scripts/generate_pptx.py --input business_plan.json --output ./output/business_plan.pptx`
...
- 命令：`python scripts/generate_pptx.py --input academic_presentation.json --output ./output/academic.pptx`
